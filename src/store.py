...
class EmbeddingStore:
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        """
        to_remove_ids = [r["id"] for r in self._store if r["metadata"].get("doc_id") == doc_id]
        if not to_remove_ids:
            return False

        to_remove_set = set(to_remove_ids)
        self._store = [r for r in self._store if r["id"] not in to_remove_set]

        if self._use_chroma and self._collection is not None:
            self._collection.delete(ids=to_remove_ids)
